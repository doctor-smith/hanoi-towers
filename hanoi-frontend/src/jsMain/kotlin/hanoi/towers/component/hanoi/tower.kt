...
fun DragDropEnvironment.Tower(
    ...
    val sizeOfMinimalSlice =
            when (slices.read().isEmpty()) {
                true -> -1
                false -> slices.read().min()
            }
    Div({ id("tower_${id}") }) {
        when (mode) {
            Mode.Play, Mode.Cheat ->
                    filled.map {
                        when (it) {
                            sizeOfMinimalSlice -> this@Tower.Slice(it, mode = mode, source = id)
                            else -> this@Tower.Slice(it, mode = mode, source = id)
                        }
                    }
            Mode.Automatic -> filled.map { this@Tower.Slice(it, mode = mode, source = id) }
        }
    }
}
...
fun Tower(
    ...
    val element: HTMLElement? =
            document.elementsFromPoint(mouse.read().coordinates.x, mouse.read().coordinates.y)
                    .find { it.id === "tower_${id}" } as
                    HTMLElement?
