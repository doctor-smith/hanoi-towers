...
val First: Parser<Char> = Parser { s ->
    when {
        s.isEmpty() -> Result(null, "")
        else -> Result(s.first(), s.drop(1))
    }
}

val FirstMatches: (Char) -> Parser<Char> = { symbol: Char ->
    First *
            { first: Char ->
                when (symbol == first) {
...
val StartsWith: (String) -> Parser<String> = { symbols ->
    symbols.map { FirstMatches(it) }.sequenceA().map { list -> list.joinToString("") { "$it" } }
}
...
val When: ((Char) -> Boolean) -> Parser<Char> = { predicate ->
    First *
            { first ->
                when (predicate(first)) {
                    true -> Parser { state -> Result(first, state) }
                    false -> Fail()
                }
            }
}
...
fun CollectWhile(predicate: (Char) -> Boolean): Parser<String> =
        (When(predicate) * { result -> CollectWhile(predicate) map { "$result$it" } }) OR
                Succeed("")
...
@Suppress("FunctionName")
fun Between(left: Char, right: Char): Parser<String> =
        FirstMatches(left) dL CollectWhile { it != right } dR FirstMatches(right)
...
fun Balance(left: Char, right: Char, pair: Pair<Int, String>): Parser<String> =
        BalanceBase(left, right) *
                { p ->
                    with((pair.first + p.first) x "${pair.second}${p.second}") {
                        if (first == 0) {
                            ReturnParser(second)
                        } else {
...
@Suppress("FunctionName")
fun Balance(left: Char, right: Char): Parser<String> =
        (FirstMatches(left) map { 1 x "$left" }) * { pair -> Balance(left, right, pair) }

@Suppress("FunctionName")
fun SplitAtFirst(separator: Char) =
        seqA(CollectWhile { it != separator }, Drop).map { list -> list[0] } *
                { start ->
                    All map { rest -> Pair(start, rest) }
                }
...
fun Split(separator: Char): Parser<List<String>> =
        Empty map
                ...
                (DropWhile { it == separator } dL
                        seqA(CollectWhile { it != separator }, Drop).map { list -> list[0] } *
                                { segment ->
                                    Split(separator) map
                                            { list ->
                                                listOf(segment, *list.toTypedArray())
                                            }
                                })
