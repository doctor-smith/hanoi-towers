...
fun Routes.merge(route: Route): Routes =
        if (route.segments.isNotEmpty()) {
            ...
            when (val segment = route.segments.first()) {
                is RouteSegment.Root ->
                        if (this.segment is RouteSegment.Root) {
...
fun Routes.append(routes: Routes): Routes =
        Routes(segment, listOf(*children.toTypedArray(), routes))

fun Routes.prepend(routes: Routes): Routes =
        Routes(segment, listOf(routes, *children.toTypedArray()))

fun Route.append(segment: String): Route =
        Route(listOf(*segments.toTypedArray(), Segment().run(segment).result!!), queryParameters)

fun Route.append(segment: RouteSegment) =
        Route(listOf(*segments.toTypedArray(), segment), queryParameters)

fun ComposableRoute.append(segment: RouteSegment) =
        ComposableRoute(listOf(*segments.toTypedArray(), segment), queryParameters, component)

fun Routes.match(path: String): ComposableRoute? =
        with(
                path
                        .dropWhile { it == '/' }
                        .dropLastWhile { it == '/' }
...
tailrec fun Routes.match(
        pair: Pair<ComposableRoute, List<String>>
): Pair<ComposableRoute?, List<String>> {
    val (route, list) = pair
...
fun ComposableRoute.parameter(name: String): String? =
        segments.filterIsInstance<RouteSegment.Variable>().find { it.name == name }?.value

@RoutingDsl
@Composable
fun Routes.compose(path: String): Boolean =
        with(match(path)) {
            if (this != null) {
                val route = this
                route.component()
